...
getPath (xs,ys) (xe,ye) a b  (d1,d2) mv =  let (x,y) = (toFloat (xe-xs),toFloat (ye-ys)) in
                                           ...
                                           ([((toFloat (xs+d1)) - a,b- (toFloat (ys+d2))),((toFloat (xe+d1))-a,b-(toFloat (ye+d2)))],mdx+(abs mv),mdy+mv,t)
sel (a,b) = if a<b then (0,round(0.5*rad),degrees 180,0.125*rad) else (0,round(-0.5*rad),degrees 0,-0.875*rad)
...
genEdge nodes (w,h) edge= let (start,end) = edge.route in
                          ...
                          let (points,mdx,mdy,th) = getPath j k (toFloat w/2) (toFloat h/2) (d0,d1) mv in
                          ...
                          let tri = ngon 3 10|>filled red|>move(mdx+d0-toFloat w/2,  (toFloat h/2)-(mdy+0.8*toFloat d1)) in
